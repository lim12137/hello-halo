...
import { AVAILABLE_MODELS, DEFAULT_MODEL, type ApiProvider } from '../../types'
...
export function ApiSetup() {
  ...
  // Form state
  const [provider, setProvider] = useState<ApiProvider>(config?.api.provider || 'anthropic')
  ...
  const handleProviderChange = (next: ApiProvider) => {
    ...
    } else if (next === 'openai' || next === 'gn') {
      // OpenAI compatible or GN
      const defaultUrl = next === 'gn' ? 'https://api.deepseek.com' : 'https://api.openai.com'
      const defaultModel = next === 'gn' ? 'deepseek-chat' : 'gpt-4o-mini'

      if (!apiUrl || apiUrl.includes('anthropic')) setApiUrl(defaultUrl)
      if (!model || model.startsWith('claude-')) setModel(defaultModel)
    }
  }
        ...
        <div className="bg-card rounded-xl p-6 border border-border">
          ...
          <div className="mb-4">
            ...
            <input
              ...
              placeholder={provider === 'anthropic' ? 'sk-ant-xxxxxxxxxxxxx' : 'sk-xxxxxxxxxxxxx'}
          ...
          <div className="mb-6">
              ...
              placeholder={
                provider === 'anthropic'
                  ...
                  : provider === 'gn'
                    ...
                    : 'https://api.openai.com or https://xx/v1'
              }
              className="w-full px-4 py-2 bg-input rounded-lg border border-border focus:border-primary focus:outline-none transition-colors"
            />
            <p className="mt-1 text-xs text-muted-foreground">
              {provider === 'anthropic'
                ...
                : t('Enter service URL (supports /v1/chat/completions)')}
            </p>
          </div>
              ...
              provider === 'anthropic' ? (
                ...
                <>
                  <input
                    ...
                    value={model}
                    onChange={(e) => setModel(e.target.value)}
                    placeholder={provider === 'gn' ? 'deepseek-chat / deepseek-reasoner' : 'gpt-4o-mini'}
                    className="w-full px-4 py-2 bg-input rounded-lg border border-border focus:border-primary focus:outline-none transition-colors"
                  />
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t('Enter service model name')}
                  </p>
                </>
              )
            }
